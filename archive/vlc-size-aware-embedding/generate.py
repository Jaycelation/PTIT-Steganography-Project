...
def main() -> None:
    ...
    p.add_argument("--step", type=float, help="QIM step. Defaults to 320 for full-res input, 224 for resized input, 160 for synthetic.")
    args = p.parse_args()
    ...
    step = args.step if args.step is not None else (320.0 if args.input and frames[0].shape[1] > 1280 else (224.0 if args.input else 160.0))
