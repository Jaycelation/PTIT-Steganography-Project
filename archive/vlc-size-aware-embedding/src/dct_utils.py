...
def parity_candidate(coef: float, bit: int, step: float) -> float:
    q = int(np.rint(coef / step))
    candidates = [q, q + 1, q - 1, q + 2, q - 2]
    candidates = [c for c in candidates if (c & 1) == bit]
    best = min(candidates, key=lambda c: abs(c * step - coef))
