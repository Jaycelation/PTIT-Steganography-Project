...
def main() -> None:
    p = argparse.ArgumentParser(description="Solve robust-dc-ac-after-reencode challenge.")
    ...
    p.add_argument("--ac-step", type=float)
    args = p.parse_args()

    cfg_path = Path(args.input).parent / "public_config.json"
    cfg = json.loads(cfg_path.read_text(encoding="utf-8")) if cfg_path.exists() else {}
    length = args.length or int(cfg["flag_length_bytes"])
    ac_step = args.ac_step or float(cfg.get("ac_step", 96.0))
    frames, _ = read_gray_video(args.input)
    answer = extract_message(frames, args.seed, length, args.repeat, ac_step)
